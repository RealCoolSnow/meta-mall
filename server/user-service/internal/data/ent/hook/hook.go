...
// The UserLogFunc type is an adapter to allow the use of ordinary
// function as UserLog mutator.
type UserLogFunc func(context.Context, *ent.UserLogMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserLogFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.UserLogMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserLogMutation", m)
	}
	return f(ctx, mv)
}
